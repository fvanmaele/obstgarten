...
Generating a single CART using the greedy algorithm, optional sampling. Being used in random forests. At the random forest the parameter m is selectable here to determine the number of randomly chosen dimensions. In addition, the algorithm always aborts at a random number of created leaves in the tree. This results in a CART that uses only a part of the given knowledge.
...
B <- sample(1:10, 1)
res <- bagging(B=B, x_train=x1, x_test=x2)
```
